...
O desenvolvimento local usa `CHITAOZINHO_AUTH_MODE=development`. O beta público
em modo `hash_only` usa `CHITAOZINHO_AUTH_MODE=anonymous`: não cria conta, não
pede e-mail e usa o identificador aleatório da sessão como capacidade de acesso.

O modo `magic_link` permanece disponível para instalações que ativem a guarda
remota de arquivos. Nesse modo, token pepper, cookies seguros, isolamento por
dono e um provedor de e-mail são obrigatórios.

## Fluxo opcional com magic link
...
Uma instalação com autenticação por e-mail pode configurar:
...
O domínio remetente precisa aparecer como verificado no Resend. A
...
remetente. O beta público atual não inicializa nenhum provedor de e-mail.

## Downloads

No modo remoto, downloads de pacotes usam uma URL bearer separada e de curta duração. O HMAC
...
Deploys públicos exigem `CHITAOZINHO_EXTENSION_IDS` com IDs Chromium exatos
...
No modo anônimo, eventos de auditoria usam somente a sessão opaca. E-mail, tokens,
