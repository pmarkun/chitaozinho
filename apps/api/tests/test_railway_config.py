...
def configs() -> tuple[dict, dict]:
    return railway.load(railway.API_PATH), railway.load(railway.WORKER_PATH)


def test_committed_railway_configs_satisfy_runtime_contract() -> None:
    api, worker = configs()
    railway.validate_configs(api, worker)
...
def test_worker_cannot_run_migrations_or_mount_evidence_volume() -> None:
    ...
    with pytest.raises(ValueError, match="must not race"):
        railway.validate_configs(api, worker_with_migration)
